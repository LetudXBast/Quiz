...
function showSection(sectionId) {
    ...
    sections.forEach(sec => {
        ...
        if (sec.id !== sectionId) {
            ...
            sec.style.opacity = "0";
            sec.style.transform = "translateX(-50%) translateY(-100%)"; /* 🔹 Cache la section */
    ...
    // 🔹 Empêcher l’accès aux documents si l'utilisateur n'est pas connecté
    if (sectionId === "documents" && !localStorage.getItem("loggedInUser")) {
        alert("Vous devez être connecté pour accéder aux documents !");
        return;
    }

    // ✅ Vérification pour éviter une réactivation inutile
 ...
 if (targetSection) {
        setTimeout(() => {
            ...
            targetSection.style.transform = "translateX(-50%) translateY(0)"; /* 🔹 Affiche la section */
...
let hideTimeout; // Stocke le timeout pour éviter la fermeture immédiate

function toggleSubcategories(listId) {
    let subcategoryList = document.getElementById(listId);
    let category = subcategoryList.parentElement; // 🔹 Récupère la catégorie parent

    let isOpen = subcategoryList.classList.contains("open");

    if (isOpen) {
        subcategoryList.classList.remove("open");
        category.classList.remove("active"); // ❌ Supprime la classe active
    } else {
        subcategoryList.classList.add("open");
        category.classList.add("active"); // ✅ Ajoute la classe active pour bordure
    }
}
// ✅ Bloquer la propagation de l’événement `click` sur les sous-catégories
document.querySelectorAll(".sidebar-subcategories li").forEach(item => {
    item.addEventListener("click", function(event) {
        event.stopPropagation(); // 🔹 Empêche la fermeture de la catégorie parent
    });
});

// 🔹 Ferme toutes les sous-catégories lorsque la souris quitte le bordereau
function closeAllSubcategories() {
    document.querySelectorAll(".sidebar-subcategories").forEach(subcategory => {
